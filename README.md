# openin — Windows 应用启动命令生成工具

openin 为指定应用生成一条可在资源管理器地址栏或终端中直接调用的启动命令（如 `vscode`、`cursor`、`claude`）。安装后，输入该命令即可以当前文件夹为参数启动对应应用，无需该应用自身支持地址栏调用。
...
### GUI 界面（推荐）
双击 `openin.exe`（或无参数运行）打开**管理主窗口**：每个应用一行（名称 + 路径框 + 浏览 + 安装 + 测试 + 卸载）。
窗口打开即**自动检索**并填入主程序路径，确认无误后点「安装」，再用「测试」一键拉起目标验证。
- 顶部菜单栏「重新检测」重新扫描路径；「深度扫描」更彻底地扫描一次（单遍遍历 + 剪枝，检出启动扫描漏掉的深层安装，
  如 `Program Files\...\bin`）；「高级▾」菜单含卸载、添加自定义
- **原生 CLI**（如 claude code，自带地址栏可用的真实二进制）：行按钮为「修复」、卸载置灰——openin 不卸载/不覆盖原生实现；
  若命令二进制缺失，「修复」会从核心程序位置复制到安装目录恢复
- **操作信息公开**：安装/卸载时，左下角状态栏逐条快速闪过具体操作步骤（写 launcher、二进制 Patch、加 PATH、
  App Paths 注册表、撤销标记等），让你看到程序在做什么
...
自动撤销安装的全部改动，卸载后系统与安装前一致：
1. 删除安装目录里的 `<name>.exe`、`<name>.cmd`
2. 清理 App Paths 注册表键（CLI 目标）
3. 仅当 openin 当时**自行加入过 PATH**、且该目录已无其他 openin 命令时，自动移除该 PATH 条目
4. 仅当安装目录由 openin 创建、且已为空时，自动删除该目录
...
- 幂等：重复安装（openin 生成的 launcher）会覆盖，不产生重复 PATH 条目；
  已存在的**非 openin 同名文件拒绝覆盖**，防止误毁真实工具
...
- **固定参数预设**：`dsh` 一键在当前文件夹启动 DeepSeek Harness Web UI——launcher 内置固定参数
  （内部用 node 直跑 dsh 的 bin.js 启动 server，绕开 npm exec 的卡死与 `cmd /c` 引号缺陷），无需任何额外输入；
  启动后**自动把当前文件夹登记为 dsh 工作区**（左侧列表可见、幂等），并自动用默认浏览器打开 Web UI
  （`http://127.0.0.1:3080`）；**首次启动弹可见控制台窗口**（可看 dsh 运行日志、Ctrl+C 或关窗停止），
  已运行时只登记工作区并打开浏览器（幂等）
