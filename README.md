...
openin 为任意应用生成一条启动命令。安装后，在地址栏或终端中输入命令名（如 `vscode`），
即可以**当前文件夹**为工作区启动对应应用——无需应用自身支持，无需手动配置环境变量。

```
  地址栏输入 vscode ──► openin 启动器（跳板）──► VS Code 打开当前目录
```
...
单文件便携程序，无需安装，双击即用。目标机器不需要编译器或任何运行时。
...
openin 在用户 bin 目录（如 `~\.local\bin`）中放置一个小型**启动器**（launcher），
它的作用很简单：读取当前工作目录，作为参数启动目标应用。
