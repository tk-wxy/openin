                                  ...
                                  int cli, const wchar_t *srcPath)
{
    ...
    /* CLI 模板: cmd /k 新建可见控制台运行 CLI(继承 cwd),并强制激活新控制台窗口 */
    static const wchar_t *CLI_TPL[] = {
        ...
        L"#define CODE_EXE L\"@@PATH@@\"",
        L"",
        L"typedef struct { HWND hwnd[64]; int count; } ConsoleList;",
        L"",
        L"static BOOL CALLBACK enum_consoles(HWND h, LPARAM lp)",
        L"{",
        L"    wchar_t cls[64];",
        L"    ConsoleList *list = (ConsoleList *)lp;",
        L"",
        L"    if (GetClassNameW(h, cls, 64) == 0) return TRUE;",
        L"    if (wcscmp(cls, L\"ConsoleWindowClass\") != 0) return TRUE;",
        L"    if (list->count < 64) list->hwnd[list->count++] = h;",
        L"    return TRUE;",
        L"}",
        L"",
        L"static BOOL in_list(const ConsoleList *l, HWND h)",
        L"{",
        L"    int i;",
        L"    for (i = 0; i < l->count; i++)",
        L"        if (l->hwnd[i] == h) return TRUE;",
        L"    return FALSE;",
        L"}",
        L"",
        L"static void activate_new_console(const ConsoleList *before)",
        L"{",
        L"    int tries;",
        L"    for (tries = 0; tries < 30; tries++) {",
        L"        ConsoleList after;",
        L"        int i;",
        L"        ZeroMemory(&after, sizeof(after));",
        L"        EnumWindows(enum_consoles, (LPARAM)&after);",
        L"        for (i = 0; i < after.count; i++) {",
        L"            if (!in_list(before, after.hwnd[i])) {",
        L"                HWND h = after.hwnd[i];",
        L"                DWORD thread = GetWindowThreadProcessId(h, NULL);",
        L"                DWORD self = GetCurrentThreadId();",
        L"                if (thread && thread != self) {",
        L"                    AttachThreadInput(self, thread, TRUE);",
        L"                    ShowWindow(h, SW_RESTORE);",
        L"                    keybd_event(VK_MENU, 0, KEYEVENTF_EXTENDEDKEY, 0);",
        L"                    keybd_event(VK_MENU, 0, KEYEVENTF_EXTENDEDKEY | KEYEVENTF_KEYUP, 0);",
        L"                    SetForegroundWindow(h);",
        L"                    SetForegroundWindow(h);",
        L"                    SetWindowPos(h, HWND_TOPMOST, 0, 0, 0, 0,",
        L"                                 SWP_NOSIZE | SWP_NOMOVE | SWP_SHOWWINDOW);",
        L"                    SetWindowPos(h, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);",
        L"                    AttachThreadInput(self, thread, FALSE);",
        L"                }",
        L"                return;",
        L"            }",
        L"        }",
        ...
        L"int wmain(int argc, wchar_t *argv[])",
        L"{",
        L"    wchar_t cmdline[2 * MAX_PATH + 512];",
        L"    STARTUPINFOW si;",
        L"    PROCESS_INFORMATION pi;",
        L"    ConsoleList before;",
        L"    MSG msg;",
        L"    int a;",
        L"",
        L"    PeekMessageW(&msg, NULL, 0, 0, PM_NOREMOVE);",
        L"",
        L"    ZeroMemory(&before, sizeof(before));",
        L"    EnumWindows(enum_consoles, (LPARAM)&before);",
        L"",
        L"    cmdline[0] = L'\\0';",
        L"    wcscat_s(cmdline, 2 * MAX_PATH + 512, L\"/k \\\"\");",
        L"    wcscat_s(cmdline, 2 * MAX_PATH + 512, CODE_EXE);",
        L"    for (a = 1; a < argc; a++) {",
        L"        wcscat_s(cmdline, 2 * MAX_PATH + 512, L\" \\\"\");",
        L"        wcscat_s(cmdline, 2 * MAX_PATH + 512, argv[a]);",
        L"        wcscat_s(cmdline, 2 * MAX_PATH + 512, L\"\\\"\");",
        L"    }",
        L"    wcscat_s(cmdline, 2 * MAX_PATH + 512, L\"\\\"\");",
        L"",
        ...
        L"    si.cb = sizeof(si);",
        L"    ZeroMemory(&pi, sizeof(pi));",
        L"    if (!CreateProcessW(L\"C:\\\\Windows\\\\System32\\\\cmd.exe\", cmdline,",
        L"                        NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {",
        L"        wchar_t msg[600];",
        L"        _snwprintf_s(msg, 600, 599, L\"Failed to start:\\n%s\\n\\nError: %lu\",",
        L"                     CODE_EXE, (unsigned long)GetLastError());",
 ...
 */
static int install_target(const wchar_t *name, const wchar_t *codeExe, int cli,
