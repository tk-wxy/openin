                                  ...
                                  int cli, const wchar_t *srcPath)
{
    ...
    /* CLI 模板: 优先用 Windows Terminal 打开可见终端运行 CLI(继承 cwd) */
    static const wchar_t *CLI_TPL[] = {
        L"#define UNICODE",
        L"#define _UNICODE",
        L"#include <windows.h>",
        L"#include <stdio.h>",
        L"#include <shellapi.h>",
        L"",
        L"#define CODE_EXE L\"@@PATH@@\"",
        L"",
        L"int wmain(int argc, wchar_t *argv[])",
        L"{",
        L"    wchar_t cwd[MAX_PATH];",
        L"    wchar_t wtPath[MAX_PATH];",
        L"    wchar_t cmdline[2 * MAX_PATH + 512];",
        L"    STARTUPINFOW si;",
        L"    PROCESS_INFORMATION pi;",
        L"    int a, n;",
        L"",
        L"    if (GetCurrentDirectoryW(MAX_PATH, cwd) == 0) cwd[0] = L'\\0';",
        L"",
        L"    /* 优先: Windows Terminal 打开可见终端运行 CLI */",
        L"    if (GetEnvironmentVariableW(L\"LOCALAPPDATA\", wtPath, MAX_PATH)) {",
        L"        wcscat_s(wtPath, MAX_PATH, L\"\\\\Microsoft\\\\WindowsApps\\\\wt.exe\");",
        L"        if (GetFileAttributesW(wtPath) != INVALID_FILE_ATTRIBUTES) {",
        L"            /* ShellExecute 是正规的「启动并激活」方式(资源管理器同样用它),",
        L"               自动处理前台焦点,无需手动 SetForegroundWindow */",
        L"            wchar_t args[2 * MAX_PATH + 512];",
        L"            n = _snwprintf_s(args, 2 * MAX_PATH + 512, 2 * MAX_PATH + 512 - 1,",
        L"                             L\"-d \\\"%s\\\" cmd /k \\\"\\\"%s\\\"\", cwd, CODE_EXE);",
        L"            for (a = 1; a < argc; a++)",
        L"                n += _snwprintf_s(args + n, 2 * MAX_PATH + 512 - n,",
        L"                                  2 * MAX_PATH + 512 - n - 1, L\" \\\"%s\\\"\", argv[a]);",
        L"            wcscat_s(args, 2 * MAX_PATH + 512, L\"\\\"\");",
        L"            if ((INT_PTR)ShellExecuteW(NULL, L\"open\", wtPath, args, NULL,",
        L"                                      SW_SHOWNORMAL) > 32)",
        L"                return 0;",
        L"        }",
        L"    }",
        ...
        L"    /* 回退: cmd /c 新建控制台 */",
        L"    cmdline[0] = L'\\0';",
        L"    wcscat_s(cmdline, 2 * MAX_PATH + 512, L\"/c \\\"\\\"\");",
