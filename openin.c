                                  ...
                                  int cli, const wchar_t *srcPath)
{
    ...
    /* CLI 模板: 命令行工具继承 cwd,不传目录参数,经 ShellExecute 启动 */
    static const wchar_t *CLI_TPL[] = {
        ...
        L"    args[0] = L'\\0';",
        L"    for (a = 1; a < argc; a++) {",
        L"        n += _snwprintf_s(args + n, 4096 - n, 4096 - n - 1,",
        L"                          L\"\\\"%s\\\" \", argv[a]);",
        L"    }",
        L"    return ((INT_PTR)ShellExecuteW(NULL, L\"open\", CODE_EXE, args, NULL,",
        L"                                   SW_SHOWNORMAL) > 32) ? 0 : 1;",
        L"}",
        L""
    };
