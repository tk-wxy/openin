                                  ...
                                  int cli, const wchar_t *srcPath)
{
    ...
    static const wchar_t *CLI_TPL[] = {
        ...
        L"            n = _snwprintf_s(cmdline, 2 * MAX_PATH + 512, 2 * MAX_PATH + 512 - 1,",
        L"                             L\"\\\"%s\\\" -d \\\"%s\\\" cmd /k \\\"\\\"%s\\\"\",",
        L"                             wtPath, cwd, CODE_EXE);",
        L"            for (a = 1; a < argc; a++)",
        L"                n += _snwprintf_s(cmdline + n, 2 * MAX_PATH + 512 - n,",
        L"                                  2 * MAX_PATH + 512 - n - 1, L\" \\\"%s\\\"\", argv[a]);",
        L"            wcscat_s(cmdline, 2 * MAX_PATH + 512, L\"\\\"\");",
        L"            ZeroMemory(&si, sizeof(si));",
        L"            si.cb = sizeof(si);",
        L"            ZeroMemory(&pi, sizeof(pi));",
        L"            if (CreateProcessW(wtPath, cmdline, NULL, NULL, FALSE,",
        L"                               0, NULL, NULL, &si, &pi)) {",
        L"                CloseHandle(pi.hThread);",
        L"                CloseHandle(pi.hProcess);",
        L"                return 0;",
