                                  ...
                                  int cli, const wchar_t *srcPath)
{
    ...
    static const wchar_t *CLI_TPL[] = {
        ...
        L"}",
        L"",
        L"/* The TCP listener can come up before dsh has finished mounting its API. */",
        L"static int dsh_rpc_succeeded(const wchar_t *response)",
        L"{",
        L"    return response && wcsstr(response, L\"\\\"ok\\\":true\") != NULL;",
        L"}",
        L"",
        L"/* Retry bounded RPCs during dsh startup instead of silently losing registration. */",
        L"static wchar_t *dsh_rpc_retry(const wchar_t *method, const wchar_t *payloadJson)",
        L"{",
        L"    DWORD start = GetTickCount();",
        L"    for (;;) {",
        L"        wchar_t *response = dsh_rpc(method, payloadJson);",
        L"        if (dsh_rpc_succeeded(response)) return response;",
        L"        free(response);",
        L"        if ((DWORD)(GetTickCount() - start) >= 12000) return NULL;",
        L"        Sleep(250);",
        L"    }",
        L"}",
        L"",
        ...
        L"",
        L"    if (webMode) {",
        L"        wcscpy_s(open_url, 1024, CODE_URL);",
        ...
        L"        }",
        L"        /* 按文件夹打开: 把当前目录登记为工作区并建一个最新会话;dsh-session-link Client 插件",
        L"           通过 URL deep-link 选中该 session;任一调用失败都降级为直接开浏览器。 */",
        L"        {",
        ...
        L"                             L\"{\\\"path\\\":\\\"%s\\\"}\", esc);",
        L"                resp = dsh_rpc_retry(L\"workspace.create\", payload);",
        L"                wsId = resp ? wcsstr(resp, L\"\\\"workspaceId\\\":\\\"\") : NULL;",
        L"                if (wsId) {",
        L"                    wchar_t id[64];",
        L"                    wchar_t sessionId[64];",
        L"                    int k;",
        L"                    wsId += 15;   /* 跳过 \"workspaceId\":\" */",
        L"                    for (k = 0; k < 63 && wsId[k] && wsId[k] != L'\"'; k++) id[k] = wsId[k];",
        L"                    id[k] = L'\\0';",
        L"                    if (k > 0) {",
        L"                        _snwprintf_s(sessionId, 64, 63, L\"openin-%lu-%lu\",",
        L"                                     (unsigned long)GetTickCount(),",
        L"                                     (unsigned long)GetCurrentProcessId());",
        L"                        _snwprintf_s(payload, 2 * MAX_PATH + 64, 2 * MAX_PATH + 63,",
        L"                                     L\"{\\\"workspaceId\\\":\\\"%s\\\",\\\"sessionId\\\":\\\"%s\\\"}\",",
        L"                                     id, sessionId);",
        L"                        free(resp);",
        L"                        resp = dsh_rpc_retry(L\"session.create\", payload);",
        L"                        if (resp) {",
        L"                            const wchar_t *sep = wcschr(CODE_URL, L'?') ? L\"&\" : L\"?\";",
        L"                            _snwprintf_s(open_url, 1024, 1023,",
        L"                                         L\"%s%sopeninSession=%s\", CODE_URL, sep, sessionId);",
        L"                        }",
        L"                    }",
        L"                }",
        L"                free(resp);",
        L"            }",
        L"        }",
        L"        ShellExecuteW(NULL, L\"open\", open_url, NULL, NULL, SW_SHOWNORMAL);",
        L"        return 0;",
        L"    }",
        L"",
        L"    /* 普通 CLI 工具: 继承当前控制台 */",
